...
gathared in views.py. Document are to be seen as completely
...
class AbstractDocument:
    ...
    def add_custom_attributes(self, key, value):
        ...
        if not isinstance(value, unicode):
            raise TypeError("custom_attribute expected as unicode")
    ...
    def share_file(self, file_path, boolean):
        """sets new value for shared file"""
        pass
        
    def tag_file(self, file_path, tag):
    ...
    def make_friend(self, pseudo, boolean):
        """sets peer as friend """
        pass

    def blacklist_peer(self, pseudo, boolean):
...
class CacheDocument(AbstractDocument):
    ...
    def add_custom_attributes(self, key, value):
        """sets new value for custom_attributes"""
        AbstractDocument.add_custom_attributes(self, key, value)
    ...
    def sets_repository(self, value):
        ...
        AbstractDocument.sets_repository(self, value)
    ...
    def share_file(self, file_path, boolean):
        """sets new value for shared file"""
        AbstractDocument.share_file(self, file_path, boolean)
        if not self.files.has_key(file_path):
            self.add_file(file_path)
        file_obj = self.files[file_path]
        file_obj.shared = boolean
        
    def tag_file(self, file_path, tag):
        """sets new value for tagged file"""
        AbstractDocument.tag_file(self, file_path, tag)
        if not self.files.has_key(file_path):
            self.add_file(file_path)
        file_obj = self.files[file_path]
        file_obj.tag = tag
    ...
    def make_friend(self, pseudo, boolean):
        """sets peer as friend """
        AbstractDocument.make_friend(self, pseudo, boolean)
        if not self.peers.has_key(pseudo):
            self.add_peer(pseudo)
        peer_obj = self.peers[pseudo]
        peer_obj.friend = boolean

    def blacklist_peer(self, pseudo, boolean):
        """sets new value for unshared file"""
        AbstractDocument.blacklist_peer(self, pseudo, boolean)
        if not self.peers.has_key(pseudo):
            self.add_peer(pseudo)
        peer_obj = self.peers[pseudo]
        peer_obj.blacklisted = boolean
