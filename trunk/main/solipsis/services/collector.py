...
class ServiceCollector(object):
            ...
            if peer_id is not None and peer_id in self.peers:
                if self.peers[peer_id].GetService(service_id) is not None:
                    for index, title in enumerate(plugin.GetPointToPointActions()):
                        def inner_func(deferred, index=index):
                            return plugin.DoPointToPointAction(index,
                                                               self.peers[peer_id],
                                                               deferred)
    ...
    def service_display_message(self, service_id, msg, title=None):
        self.ui.display_message(service_id + ": " + msg, title=title)

    def service_display_status(self, service_id, msg):
        self.ui.display_status(service_id + ": " + msg)
